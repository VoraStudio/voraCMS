...
(Previously: all requests under `/api/*` required JWT Bearer authentication.)
...
(Previously: the payload included `allowed_domains` but had no restriction on roles for public tokens.)
